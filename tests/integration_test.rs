...
fn test_example_todos_file() {
    ...
    let data_path = PathBuf::from(manifest_dir).join("data").join("example_todos.txt");
    
    // Ensure the test data file exists
    assert!(data_path.exists(), "Test data file should exist at {:?}", data_path);
...
fn test_todo_extraction_from_data_file() {
    ...
    assert!(content.contains("TODO #4949"), "Should contain local GitLab issue format");
    assert!(content.contains("TODO github.com/knope-dev/knope#1686"), "Should contain GitHub shorthand format");
    assert!(content.contains("TODO github.com/rust-lang/rust/issues/1"), "Should contain GitHub full URL format");
    assert!(content.contains("TODO rigetti/qcs/magneto#229"), "Should contain GitLab rendered format");
    assert!(content.contains("TODO github.com/doesnotexist/knope#1"), "Should contain non-existent GitHub issue");
}
