...
fn test_open_gitlab_issue() {
    let gitlab_token = std::env::var("GITLAB_TOKEN")
        .expect("GITLAB_TOKEN must be set for this test");
    ...
    std::fs::write(&test_file, "// TODO rigetti/experimental/kstrand/todo-curator#1\n")
    ...
    assert!(output.status.success(), "Command should succeed for open issue");
}
...
fn test_closed_github_issue() {
    let gh_token = std::env::var("GH_TOKEN")
        .expect("GH_TOKEN must be set for this test");
    ...
    assert!(!output.status.success(), "Command should fail when closed issues are found");
}
...
fn test_nonexistent_github_issue() {
    ...
    std::fs::write(&test_file, "// TODO github.com/nonexistent-user-12345/nonexistent-repo-67890#99999\n")
        .expect("Failed to write test file");
    ...
    // Should exit with error code
    assert!(!output.status.success(), "Command should fail when non-existent issues are found");
}
...
fn test_nonexistent_gitlab_issue() {
    ...
    std::fs::write(&test_file, "// TODO rigetti/experimental/kstrand/todo-curator#999999\n")
        .expect("Failed to write test file");
...
fn test_mixed_issue_states() {
    ...
    assert!(!output.status.success(), "Command should fail when problems are found");
}
