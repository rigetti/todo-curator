...
//! Run with:
//! ```bash
//! # Use the helper script to populate auth env vars using `glab` and `gh` CLI tools
//! source scripts/auth-setup.sh
//!
//! # GitHub tests only
//! cargo test --test api_integration_test --features test-integration-github
//!
//! # GitLab tests only
//! cargo test --test api_integration_test --features test-integration-gitlab
//!
//! # All tests (combined)
//! cargo test --test api_integration_test --features test-integration-github,test-integration-gitlab
//! ```
...
async fn test_closed_github_issue() {
    ...
    match &closed_issue.reference.kind {
        TodoReferenceKind::GitHubIssueOrPr { repo, number, .. } => {
            ...
            assert_eq!(number, &1, "Should be pull-request #1");
        }
        _ => panic!(
            "Expected GitHubIssueOrPr, got: {:?}",
...
/// Test that the tool correctly identifies a closed GitHub pull request in qcs-sdk-rust
/// Uses rigetti/qcs-sdk-rust#1 which is a pull request, not an issue, and is closed
#[tokio::test]
#[cfg(feature = "test-integration-github")]
async fn test_closed_github_pull_request_qcs_sdk_rust() {
    let _gh_token = std::env::var("GITHUB_TOKEN").expect("GITHUB_TOKEN must be set for this test");

    // Create a temporary test file with a TODO referencing a closed pull request
    let temp_dir = std::env::temp_dir();
    let test_file = temp_dir.join("test_closed_github_pull_request_qcs_sdk_rust.rs");
    std::fs::write(&test_file, "// TODO github.com/rigetti/qcs-sdk-rust#1:\n")
        .expect("Failed to write test file");
    ...
    // The pull request is closed, so it should appear in closed issues
    assert!(
        !result.closed.is_empty(),
        "Closed pull request should be reported. Got: {:?}",
        result
    );
    ...
    assert_eq!(
        result.closed.len(),
        1,
        "Should have exactly one closed pull request"
    );

    // Verify the specific pull request is rigetti/qcs-sdk-rust#1
    let closed_issue = &result.closed[0];
    match &closed_issue.reference.kind {
        TodoReferenceKind::GitHubIssueOrPr { repo, number, .. } => {
            assert_eq!(
                repo.as_deref(),
                Some("rigetti/qcs-sdk-rust"),
                "Should be rigetti/qcs-sdk-rust repo"
            );
...
async fn test_mixed_issue_states() {
    ...
    match &not_found_ref.reference.kind {
        ...
        _ => panic!(
            "Expected GitHubIssueOrPr for not_found, got: {:?}",
