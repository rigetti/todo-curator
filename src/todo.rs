...
pub enum TodoReference {
    ...
    GitLabMr { project: Option<String>, number: u32, source_line: String, file_path: String, line_number: u64 },
    GitHubPr { repo: String, number: u32, source_line: String, file_path: String, line_number: u64 },
}
...
type ExtractorFn = Box<dyn Fn(&regex::Captures, &str, &str, u64) -> Option<TodoReference> + Send + Sync>;
...
impl TodoExtractor {
        ...
        let patterns: Vec<(Regex, ExtractorFn)> = vec![
            ...
            (
                ...
                Box::new(|caps: &regex::Captures, line: &str, file_path: &str, line_number: u64| {
                    let repo = caps.get(1)?.as_str().to_string();
                    let number = caps.get(2)?.as_str().parse::<u32>().ok()?;
                    Some(TodoReference::GitHubPr { repo, number, source_line: line.trim().to_string(), file_path: file_path.to_string(), line_number })
                }),
            ),
    ...
    pub fn extract_from_directory(&self, dir: &Path) -> anyhow::Result<HashSet<TodoReference>> {
        tracing::debug!("Extracting TODO references from directory: {}", dir.display());
